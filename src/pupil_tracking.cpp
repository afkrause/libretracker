...
void Pupil_tracking::run(enum_simd_variant simd_width, int eye_cam_id)
{
	...
	sg.add_radio_button("PuRe (for research only!)", [&, s = simd_width]() {setup(s, PUPIL_TRACKING_PURE); }, 1, 0, "PuRe is a high accuracy- and performance algorithm from the university of t�bingen. License: research only! You are not allowed to use this algorithm and its code for commercial applications.");
	auto button = sg.add_radio_button("PuReST (for research only!)", [&, s = simd_width]() {setup(s, PUPIL_TRACKING_PUREST); }, 1, 0, "PuReST is a high accuracy - and performance algorithm from the university of t�bingen.License: research only!You are not allowed to use this algorithm and its code for commercial applications.");
	button->value(true);
