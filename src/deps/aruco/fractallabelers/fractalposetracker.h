...
namespace aruco {
    ...
    public:
         ...
         */
        void ROI(const std::vector<cv::Mat> imagePyramid, cv::Mat &img, std::vector<cv::Point2f> &innerPoints2d, cv::Point2f &offset, float &ratio);
         ...
         */
        cv::Mat fractal_solve_ransac(int ninners, std::vector<std::pair<uint, std::vector<uint>>> inner_kpnt, std::vector<cv::KeyPoint> kpnts, uint32_t maxIter=500, float _minInliers=0.35f, float _thresInliers=0.7f);
        ...
        // return all inner points from fractal marker
